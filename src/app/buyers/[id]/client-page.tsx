...
import { Mail, MapPin, CalendarDays, ShoppingBag, LogIn, Loader2 } from "lucide-react";
import { format } from "date-fns";
import { Badge } from "@/components/ui/badge";
import type { Buyer, SourcingRequest } from "@/lib/types";
...
interface BuyerProfileClientPageProps {
  buyer: Buyer;
  sourcingRequests: SourcingRequest[];
}

export function BuyerProfileClientPage({ buyer, sourcingRequests }: BuyerProfileClientPageProps) {
  ...
  const renderContactButton = () => {
    ...
    return (
       <Button asChild variant="secondary">
          <Link href={`/login?redirect=${pathname}`}><LogIn className="mr-2 h-4 w-4" /> Login to Contact Buyer</Link>
       </Button>
    )
  }
